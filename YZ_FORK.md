...
当前开发源码版本为 `v26.8.1`，尚未发布。已发布的 YZboard-Node `v1.17.0` 仍固定消费 `v26.8.0` / `9fcf874e21147978c5117e4832c78b4adadd4320`，不会自动包含本次补丁。产品版本独立于 Xray 上游版本；历史版本保持原名。Go 模块消费者应固定到验证后的完整提交，不能仅使用这里的源码版本说明。

## `v26.8.1` REALITY 双模式兼容（未发布）
